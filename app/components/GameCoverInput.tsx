...
export function GameCoverInput({ value, file, onChange, onFileChange, onEditingChange, disabled = false, chinese }: GameCoverInputProps) {
  const id = useId();
  ...
  useEffect(() => {
    onEditingChange(Boolean(source) || loading || processing);
  }, [source, loading, processing, onEditingChange]);
  useEffect(() => {
  ...
  return (
    <div className="space-y-3" role="group" aria-labelledby={`${id}-label`} aria-busy={loading || processing}>
      ...
      {imageFailed && !source && <p className="text-sm text-destructive">{say('圖片無法顯示，請確認網址或改用上傳。', 'Image cannot be displayed. Check the URL or upload a file.')}</p>}
      {file && !source && <Button type="button" variant="ghost" size="sm" disabled={busy} onClick={() => onFileChange(null)}>
        {say('放棄新封面，恢復原設定', 'Discard new cover and restore original setting')}
      </Button>}
      <details className="rounded-lg border p-3">
        <summary className="cursor-pointer text-sm">{say('或使用圖片網址', 'Or use an image URL')}</summary>
        <label htmlFor={`${id}-url`} className="mt-3 block text-xs text-muted-foreground">{say('縮圖網址', 'Thumbnail URL')}</label>
        <Input id={`${id}-url`} type="url" value={value} placeholder="https://…" disabled={busy || Boolean(source)} className="mt-1"
          onChange={event => { onFileChange(null); onChange(event.target.value); setError(''); }} />
      </details>
      <p className="text-xs text-muted-foreground">{say('封面會公開顯示，請勿上傳個資或私密內容。', 'Covers are public. Do not upload personal or private information.')}</p>
