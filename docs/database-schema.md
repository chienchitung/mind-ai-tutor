...
### `game_assignments`
把一款數位遊戲指派給特定班級的教學活動。每次指派都有獨立 `id`，班級專屬網址會帶入 `?assignment=<id>`。因此同一個 `digital_games.id` 同時提供給兩個班級時，學習紀錄與排行榜仍可依 `game_assignment_id` 分開分析。沒有 assignment 的舊網址與歷史資料繼續使用，但會保持「未分類」，不會自動猜測班級。
