...
### `students`
老師的學生名冊。`external_id` 保存學校既有的學號或研究編號；學生從班級專屬活動網址進入後，輸入這個自己已知的編號即可連結身分，不需要註冊網站帳號。`login_code` 保留作為忘記編號或特殊狀況使用的備用登入碼。`email` 為選填欄位。

### `classrooms` / `classroom_students`
老師建立的正式班級與班級名單。學生與班級是多對多關係，同一位學生可以加入不同學期或不同研究群組。`study_label` 用來標示同一項實驗，`study_arm` 可填「實驗組」、「對照組」或其他分組名稱；這兩個欄位只負責研究標記，不會改變學生權限。

日常操作以封存為主；封存會保留名單、活動與報告並可重新啟用。永久移除只允許用於已封存班級：刪除班級、名單關係及活動指派，但學生與學習紀錄保留，紀錄上的 `game_assignment_id` 會因外鍵 `ON DELETE SET NULL` 而失去班級分類。

### `game_assignments`
把一款數位遊戲指派給特定班級的教學活動。每次指派都有獨立 `id`，班級專屬網址會帶入 `?assignment=<id>`。因此同一個 `digital_games.id` 同時提供給兩個班級時，學習紀錄與排行榜仍可依 `game_assignment_id` 分開分析。若學生從一般遊戲網址登入，系統只會在該學生對這款遊戲恰好有一個有效班級活動時自動補上 assignment；沒有符合活動時保持未分類，同時符合多個活動時則要求使用班級專屬連結，避免誤歸班級。既有歷史資料不會被回填猜測。
...
### `learning_records`
**已連結身份**的學生完成某一關的紀錄（開始/完成時間、花費秒數、提交答案次數）。學生可在班級專屬連結輸入學號／研究編號，或使用備用登入碼來連結名冊；訪客模式完全不會寫進來。從班級專屬連結進入時會一併保存 `game_assignment_id`，資料庫觸發器會驗證遊戲與班級相符，而且學生仍是該班級的有效成員。
